...
/**
 * A model of neural networks in libMentha
 *
 ...
 *
 *     using namespace lib_mentha;
 *     using namespace lib_mentha::layers;
 *
...
class network {
 public:
  ...
  void save(std::ostream &os) const {
    os.precision(std::numeric_limits<lib_mentha::float_t>::digits10);
    net_.save(os);
  }

  ///< @deprecated use load(filename,target,format) instead.
  void load(std::istream &is) {
    is.precision(std::numeric_limits<lib_mentha::float_t>::digits10);
    net_.load(is);
  }
