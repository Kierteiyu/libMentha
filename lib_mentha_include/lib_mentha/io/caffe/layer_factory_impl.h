...
typedef lib_mentha::shape3d shape_t;
...
inline std::shared_ptr<layer> create_ave_pool(layer_size_t pool_size_w,
  ...
  // libMentha has trainable parameter in average-pooling layer
...
inline bool layer_match(const std::string &caffetype,
                        const std::string &lib_mentha_type) {
  ...
  for (size_t i = 0; i < sizeof(conversions) / sizeof(conversions[0]); i++) {
    if (conversions[i][0] == caffetype && conversions[i][1] == lib_mentha_type)
      return true;
  }
  return false;
}
