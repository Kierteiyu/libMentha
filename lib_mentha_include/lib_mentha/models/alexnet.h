...
class alexnet : public lib_mentha::network<lib_mentha::sequential> {
 public:
  explicit alexnet(const std::string &name = "")
    : lib_mentha::network<lib_mentha::sequential>(name) {
    // todo: (karandesai) shift this to lib_mentha::activation
    using relu     = lib_mentha::activation::relu;
    using conv     = lib_mentha::layers::conv;
    using max_pool = lib_mentha::layers::max_pool;
