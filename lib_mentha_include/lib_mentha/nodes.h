...
void save(Archive &ar, const std::vector<lib_mentha::layer *> &v) {
#ifndef CNN_NO_SERIALIZATION
  ar(cereal::make_size_tag(static_cast<cereal::size_type>(v.size())));
  for (auto n : v) {
    lib_mentha::layer::save_layer(ar, *n);
  }
#else
  throw lib_mentha::nn_error("libMentha was not built with Serialization support");
#endif  // CNN_NO_SERIALIZATION
}

template <typename Archive>
void load(Archive &ar, std::vector<std::shared_ptr<lib_mentha::layer>> &v) {
#ifndef CNN_NO_SERIALIZATION
  cereal::size_type size;
  ar(cereal::make_size_tag(size));

  for (size_t i = 0; i < size; i++) {
    v.emplace_back(lib_mentha::layer::load_layer(ar));
  }
#else
  throw lib_mentha::nn_error("libMentha was not built with Serialization support");
#endif  // CNN_NO_SERIALIZATION
 ...
 * Node is a computational unit of libMentha (for example, convolution).
 ...
 * given node alive. If given type is lvalue, libMentha holds raw-pointer only
...
#else
      throw nn_error("libMentha was not built with Serialization support");
#endif  // CNN_NO_SERIALIZATION
    }
